    ...
    include: {
      publishedVersion: {
        include: {
          ...
          projects: { where: { project: { status: ContentStatus.PUBLISHED } }, select: { project: { select: { id: true, publishedVersion: { select: { title: true, slug: true, shortDescription: true, city: true, district: true, coverMedia: { select: { url: true, altText: true } }, category: { select: { name: true, iconKey: true, isActive: true } } } } } } } },
          ...
          faqs: { where: { faq: { status: ContentStatus.PUBLISHED } }, select: { faq: { select: { id: true, publishedVersion: { select: { question: true, answer: true } } } } } },
        },
      },
    },
  });
