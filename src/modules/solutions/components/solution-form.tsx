    ...
    <form action={formAction} noValidate ref={formRef}>
      ...
      <header className="mb-6 border-b border-border pb-5">
        ...
        <div className="mt-2 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">{solution ? <Link className={cn(buttonVariants({ variant: "outline" }), "min-h-10")} href={`/preview/solutions/${solution.id}`} target="_blank">معاينة<ExternalLink /></Link> : null}<ServiceEditorSubmit kind="save" /><ServiceEditorSubmit kind="publish" /></div>
        </div>
      </header>
      ...
      <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,820px)_300px]" key={state.revision}>
        ...
        <aside className="grid min-w-0 gap-3 [&>details]:order-2 xl:sticky xl:top-20 xl:[&>details]:order-3">
          ...
          <details className="group rounded-xl border border-border bg-card" open={hasSeoError(state) || undefined}>
            <summary className="flex min-h-12 cursor-pointer list-none items-center justify-between rounded-xl px-4 text-sm font-semibold outline-none transition-colors hover:bg-dashboard-hover focus-visible:ring-2 focus-visible:ring-ring marker:content-none">تهيئة محركات البحث<ChevronDown className="size-4 text-muted-foreground transition-transform group-open:rotate-180" /></summary>
            <div className="grid gap-4 border-t border-border p-4">
              <CmsFieldShell error={fieldError(state, "seoTitle")} id="seoTitle" label="عنوان SEO"><CmsInput aria-invalid={Boolean(fieldError(state, "seoTitle"))} id="seoTitle" name="seoTitle" defaultValue={values.seoTitle} /></CmsFieldShell>
              <CmsFieldShell error={fieldError(state, "seoDescription")} id="seoDescription" label="وصف SEO"><CmsTextarea aria-invalid={Boolean(fieldError(state, "seoDescription"))} id="seoDescription" name="seoDescription" defaultValue={values.seoDescription} /></CmsFieldShell>
              <CmsFieldShell error={fieldError(state, "canonicalUrl")} id="canonicalUrl" label="الرابط القانوني"><CmsInput aria-invalid={Boolean(fieldError(state, "canonicalUrl"))} dir="ltr" id="canonicalUrl" name="canonicalUrl" defaultValue={values.canonicalUrl} /></CmsFieldShell>
