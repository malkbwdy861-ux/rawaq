...
export function ServiceRelationSelector({ name, label, options, selectedIds = [] }: { name: string; label: string; options: CmsRelationOption[]; selectedIds?: string[] }) {
  ...
  return (
    <details className="group border-b border-border last:border-b-0">
      ...
      <div className="pb-4">
        <label className="relative block" htmlFor={searchId}><span className="sr-only">بحث في {label}</span><Search className="pointer-events-none absolute start-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input className="h-10 rounded-[4px] border-border bg-background ps-9 pe-3 text-sm focus-visible:border-primary focus-visible:ring-ring/25" id={searchId} onChange={(event) => setQuery(event.target.value)} placeholder={`بحث في ${label}`} type="search" value={query} /></label>
