...
export function ServicesPagination({ pagination, query }: { pagination: CmsPagination; query: Record<string, string | number | undefined> }) {
  ...
  return (
    <nav aria-label="صفحات الخدمات" className="flex flex-col gap-3 border-t border-border bg-card px-4 py-3 text-sm sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex items-center gap-1">
        <PageLink disabled={!page.hasPreviousPage} href={pageHref({ ...query, page: page.page - 1 })} label="الصفحة السابقة"><ChevronRight /></PageLink>
        ...
        <PageLink disabled={!page.hasNextPage} href={pageHref({ ...query, page: page.page + 1 })} label="الصفحة التالية"><ChevronLeft /></PageLink>
      </div>
    </nav>
  );
}
...
function pageHref(query: Record<string, string | number | undefined>) {
  ...
  return search ? `/dashboard/services?${search}` : "/dashboard/services";
}
