    ...
    <div className="flex flex-col gap-3 border-b border-border bg-card px-3 py-3 sm:flex-row sm:items-center">
      <div className="flex min-w-0 flex-1 flex-col gap-2 sm:flex-row">
        <label className="relative min-w-0 flex-1 sm:max-w-md">
          <span className="sr-only">البحث في {searchLabel}</span>
          ...
          <Input className="h-10 border-border-strong bg-card ps-9 pe-9 text-sm focus-visible:border-primary focus-visible:ring-ring/25" onChange={(event) => setSearch(event.target.value)} placeholder={`البحث في ${searchLabel}...`} type="search" value={search} />
        ...
        <label className="grid gap-1 sm:w-48">
          ...
          <Select onValueChange={(value) => navigate(search, value as ServiceStatusFilter)} value={status}>
            ...
            <SelectContent align="end">
              {statusOptions.map((option) => <SelectItem key={option} value={option}>{option === "ALL" ? allLabel : statusLabels[option]} ({statusCounts[option].toLocaleString("ar-SA")})</SelectItem>)}
            </SelectContent>
          </Select>
        </label>
