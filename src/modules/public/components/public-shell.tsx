...
export function PublicShell({ children, settings }: { children: ReactNode; settings: PublicSettings | null }) {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <footer className="public-footer overflow-hidden text-primary-foreground">
          ...
          <div className="grid gap-9 lg:grid-cols-[minmax(0,0.27fr)_minmax(0,0.45fr)_minmax(0,0.28fr)] lg:gap-10 xl:gap-12">
            <section aria-label="عن الشركة" className="order-1 max-w-[32rem]">
              <Link aria-label={`${name}، الرئيسية`} className="inline-flex min-w-0 items-center gap-3" href="/">
                ...
                <span className="min-w-0">
                  <strong className="block truncate text-lg font-bold leading-[1.45] text-primary-foreground lg:text-xl">{name}</strong>
                  <span className="mt-0.5 block text-xs font-semibold text-primary-soft/80">تظليل وتنفيذ خارجي</span>
                </span>
              </Link>

              {settings?.companyDescription ? <p className="mt-4 max-w-[42ch] text-sm leading-7 text-primary-soft/80">{settings.companyDescription}</p> : null}

              {socialLinks.length ? <div className="mt-5 flex flex-wrap gap-2" aria-label="روابط التواصل الاجتماعي">
                {socialLinks.map(({ href, label, mark }) => <a aria-label={label} className="grid size-10 place-items-center rounded-[9px] border border-primary-soft/20 bg-primary/20 text-[0.72rem] font-semibold text-primary-soft transition-colors duration-150 hover:border-primary-soft/45 hover:bg-primary/35 hover:text-primary-foreground" href={href} key={label} rel="noreferrer" target="_blank"><span aria-hidden="true" dir="ltr">{mark}</span></a>)}
              </div> : null}
            </section>
            ...
            <nav aria-label="روابط التذييل" className="order-3 grid gap-7 sm:grid-cols-2 lg:order-2 lg:grid-cols-3 lg:gap-6">
              {footerNavigationGroups.map((group) => <section className="min-w-0" key={group.title}>
                <h2 className="text-sm font-semibold text-primary-foreground">{group.title}</h2>
                <ul className="mt-3.5 grid gap-2.5 text-sm">
                  {group.links.map(([label, href]) => <li key={`${group.title}-${label}`}><Link className="inline-flex min-h-8 items-center font-normal text-primary-soft/78 transition-colors duration-150 hover:text-primary-foreground" href={href}>{label}</Link></li>)}
                </ul>
              </section>)}
            </nav>

            <section aria-labelledby="footer-contact-title" className="order-2 border-y border-primary-soft/15 py-6 lg:order-3 lg:self-start lg:border-y-0 lg:border-s lg:py-0 lg:ps-8">
              <h2 className="text-base font-semibold leading-[1.5] text-primary-foreground" id="footer-contact-title">تواصل معنا واطلب الخدمة</h2>
              <p className="mt-2 max-w-[38ch] text-sm leading-7 text-primary-soft/78">شاركنا نوع التظليل والموقع، ونحدد لك الخطوة المناسبة للتنفيذ.</p>

              <div className="mt-5 grid gap-3 sm:max-w-[21rem]">
                <a className="inline-flex min-h-12 items-center justify-center gap-2 rounded-[9px] bg-primary-foreground px-5 text-sm font-semibold text-primary-active transition-colors duration-150 hover:bg-primary-soft focus-visible:outline-ring" href={whatsappHref}><MessageCircle aria-hidden="true" className="size-[18px]" strokeWidth={1.8} />تواصل عبر واتساب</a>
                {settings?.primaryPhone ? <a className="inline-flex min-h-12 items-center justify-center gap-2 rounded-[9px] border border-primary-soft/45 px-5 text-sm font-semibold text-primary-foreground transition-colors duration-150 hover:border-primary-soft hover:bg-primary/35 focus-visible:outline-primary-soft" dir="ltr" href={phoneHref(settings.primaryPhone)}><Phone aria-hidden="true" className="size-[18px]" strokeWidth={1.8} /><bdi>{settings.primaryPhone}</bdi></a> : null}
              </div>

              <p className="mt-5 flex max-w-[38ch] items-start gap-2 text-sm leading-7 text-primary-soft/78"><MapPin aria-hidden="true" className="mt-1 size-[18px] shrink-0 text-primary-soft" strokeWidth={1.7} />{settings?.address || "جدة، المملكة العربية السعودية"}</p>
            </section>
          </div>

          <div className="mt-9 border-t border-primary-soft/15 pt-5 text-sm text-primary-soft/72 lg:mt-10">
