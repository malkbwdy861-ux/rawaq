...
export function WhatsappQuickDialog({ whatsappNumber, defaultMessage }: { whatsappNumber: string; defaultMessage?: string | null }) {
  ...
  return <>
    ...
    <dialog className="whatsapp-dialog m-auto max-h-[calc(100dvh_-_2rem)] w-[calc(100%_-_2rem)] max-w-[500px] overflow-y-auto rounded-[16px] border border-border bg-card p-0 text-foreground shadow-[0_24px_64px_oklch(22%_0.018_155_/_0.20)] backdrop:bg-[color-mix(in_oklch,var(--foreground)_48%,transparent)]" onClick={(event) => { if (event.target === event.currentTarget) event.currentTarget.close(); }} onClose={() => setSelectedAction("")} ref={dialogRef}>
