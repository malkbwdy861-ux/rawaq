...
export function MediaUploadDialog() {
  ...
  function selectFiles(fileList?: FileList | null) {
    ...
    setError("");
    updatePreviews([...previews, ...files.map((file) => ({ file, name: file.name, url: URL.createObjectURL(file) }))]);
  }
  ...
  return (
    <>
      ...
      <dialog aria-describedby={descriptionId} aria-labelledby={titleId} className="fixed inset-0 m-auto max-h-[calc(100vh-2rem)] w-[680px] max-w-[calc(100vw-2rem)] overflow-hidden rounded-xl border border-border bg-card p-0 text-foreground shadow-[var(--shadow-float)] backdrop:bg-foreground/30" onClick={(event) => { if (event.target === event.currentTarget) closeDialog(); }} ref={dialogRef}>
        <form action={uploadMediaAction} className="flex max-h-[calc(100vh-2rem)] flex-col" onSubmit={(event) => { if (error || !inputRef.current?.files?.length) event.preventDefault(); }}>
          ...
          <div className="min-h-0 overflow-y-auto p-5">
            {selectedCount ? <div className="grid min-h-56 place-items-center overflow-hidden rounded-xl border border-dashed border-border-strong bg-dashboard-canvas/45 text-center"><div className="grid w-full gap-3 p-3"><div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{previews.map((preview, index) => <span className="relative block aspect-square overflow-hidden rounded-lg bg-secondary" key={preview.url}><Image alt={`معاينة الصورة رقم ${index + 1}`} className="object-contain p-2" fill sizes="150px" src={preview.url} unoptimized /><button aria-label={`إزالة ${preview.name}`} className="absolute end-1.5 top-1.5 grid size-7 place-items-center rounded-full bg-destructive text-white shadow-sm outline-none transition-all duration-200 ease-out hover:scale-105 hover:bg-destructive/90 hover:text-white focus-visible:ring-2 focus-visible:ring-ring" onClick={() => removeFile(index)} type="button"><X className="size-3.5 text-white" /></button></span>)}</div><div className="min-w-0 text-center"><bdi className="block truncate text-sm font-medium" dir="ltr">{selectedCount === 1 ? previews[0]?.name : `${selectedCount.toLocaleString("ar-SA")} صور محددة`}</bdi><label className="mt-2 inline-flex cursor-pointer text-xs font-semibold text-primary transition-colors hover:text-primary/80" htmlFor={inputId}>اختيار صور أخرى</label></div></div></div> : <label className="grid min-h-56 cursor-pointer place-items-center overflow-hidden rounded-xl border border-dashed border-border-strong bg-dashboard-canvas/45 text-center outline-none transition-colors hover:border-primary hover:bg-primary-soft/30 focus-within:border-primary focus-within:ring-2 focus-within:ring-ring/25" htmlFor={inputId}><span className="px-6"><span className="mx-auto grid size-12 place-items-center rounded-xl bg-primary-soft text-primary"><ImageIcon className="size-6" /></span><strong className="mt-4 block text-sm">اختيار صور من الجهاز</strong><span className="mt-2 block text-xs leading-5 text-muted-foreground">صورة واحدة أو عدة صور، JPEG أو PNG أو WebP، بحد أقصى {mediaConfig.maxUploadBytes / 1024 / 1024} ميجابايت لكل صورة</span></span></label>}
            <Input accept={mediaConfig.allowedMimeTypes.join(",")} className="sr-only" id={inputId} multiple name="files" onChange={(event) => selectFiles(event.target.files)} ref={inputRef} required type="file" />
