...
export function MediaDeleteDialog({ item, label, onOpen }: { item: Pick<MediaEditItem, "id" | "originalFilename">; label?: string; onOpen?: () => void }) {
  ...
  return <><Button aria-label={`حذف ${item.originalFilename}`} className={label ? "min-h-9 w-full justify-start px-2 text-destructive hover:bg-danger-soft hover:text-destructive" : "size-9 min-h-9 text-destructive hover:bg-danger-soft hover:text-destructive"} onClick={() => { dialogRef.current?.showModal(); onOpen?.(); }} size={label ? "sm" : "icon"} title="حذف الصورة" type="button" variant="ghost"><Trash2 />{label}</Button><dialog aria-describedby={descriptionId} aria-labelledby={titleId} className="m-auto w-[min(calc(100%-2rem),440px)] rounded-xl border border-border bg-card p-0 text-foreground shadow-[var(--shadow-dialog)] backdrop:bg-foreground/30" onClick={(event) => { if (event.target === event.currentTarget) closeDialog(); }} ref={dialogRef}><form action={deleteMediaAction} className="p-5"><input name="id" type="hidden" value={item.id} /><span className="grid size-10 place-items-center rounded-md bg-danger-soft text-destructive"><Trash2 className="size-5" /></span><h2 className="mt-3 text-lg font-semibold" id={titleId}>حذف الصورة؟</h2><p className="mt-1.5 text-sm leading-6 text-text-secondary" id={descriptionId}>قد تكون الصورة مستخدمة في أجزاء أخرى من الموقع. سيتم حذفها وإزالة ارتباطاتها تلقائياً.</p><p className="mt-3 truncate rounded-md border border-border bg-secondary px-3 py-2 text-sm font-medium" dir="ltr" title={item.originalFilename}>{item.originalFilename}</p><div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><Button onClick={closeDialog} type="button" variant="secondary">إلغاء</Button><DeleteSubmit /></div></form></dialog></>;
}
