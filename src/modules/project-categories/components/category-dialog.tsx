...
export function ProjectCategoryDialog({ category, compact = false }: { category?: CategoryRecord; compact?: boolean }) {
  ...
  return <>
    ...
    <dialog aria-describedby={descriptionId} aria-labelledby={titleId} className="m-auto max-h-[calc(100dvh-2rem)] w-[min(calc(100%-2rem),680px)] overflow-y-auto rounded-xl border border-border bg-card p-0 text-foreground shadow-[var(--shadow-dialog)] backdrop:bg-foreground/30" onCancel={(event) => { if (pending) event.preventDefault(); }} onClick={(event) => { if (event.target === event.currentTarget) close(); }} onClose={() => triggerRef.current?.focus()} ref={dialogRef}>
...
export function DeleteProjectCategoryDialog({ category }: { category: Pick<CategoryRecord, "id" | "name"> }) {
  ...
  const titleId = useId();
  const descriptionId = useId();
  return <><Button aria-label={`حذف ${category.name}`} className="size-9 min-h-9 text-destructive hover:bg-danger-soft hover:text-destructive" onClick={() => dialogRef.current?.showModal()} size="icon" title="حذف التصنيف" type="button" variant="ghost"><Trash2 /></Button><dialog aria-describedby={descriptionId} aria-labelledby={titleId} className="m-auto w-[min(calc(100%-2rem),440px)] rounded-xl border border-border bg-card p-0 text-foreground shadow-[var(--shadow-dialog)] backdrop:bg-foreground/30" onClick={(event) => { if (event.target === event.currentTarget) dialogRef.current?.close(); }} ref={dialogRef}><form action={deleteProjectCategoryAction} className="p-5"><input name="categoryId" type="hidden" value={category.id} /><span className="grid size-10 place-items-center rounded-md bg-danger-soft text-destructive"><Trash2 className="size-5" /></span><h2 className="mt-3 text-lg font-semibold" id={titleId}>حذف تصنيف المشروع</h2><p className="mt-1.5 text-sm leading-6 text-text-secondary" id={descriptionId}>سيُلغى إسناد هذا التصنيف من المشاريع الحالية، ولن تُحذف أي مشاريع.</p><p className="mt-3 rounded-md border border-border bg-secondary px-3 py-2 text-sm font-medium">{category.name}</p><div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><Button onClick={() => dialogRef.current?.close()} type="button" variant="secondary">إلغاء</Button><Button type="submit" variant="destructive">تأكيد حذف التصنيف</Button></div></form></dialog></>;
}
