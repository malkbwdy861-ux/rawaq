...
export async function createPageDraftAction(formData: FormData) {
  ...
  if (existing) {
    if (!existing.draftVersionId) {
      const draft = await prisma.pageVersion.create({ data: { pageId: existing.id } });
      await prisma.page.update({ where: { id: existing.id }, data: { draftVersionId: draft.id } });
    }
    redirect(`/dashboard/pages/${parsed.data}`);
  }
  const page = await prisma.page.create({ data: { key: parsed.data, versions: { create: {} } }, include: { versions: { select: { id: true }, take: 1 } } });
  await prisma.page.update({ where: { id: page.id }, data: { draftVersionId: page.versions[0]?.id } });
...
export async function savePageDraftAction(formData: FormData) {
  ...
  await saveDraft(identity.data.pageId, parsed.data.data, parsed.data.seo);
  revalidatePagePaths(identity.data.key);
...
export async function publishPageAction(formData: FormData) {
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      const page = await tx.page.findUnique({ where: { id: identity.data.pageId }, select: { draftVersionId: true } });
      if (!page?.draftVersionId) throw new Error("لا توجد مسودة قابلة للنشر.");
      const published = await tx.pageVersion.create({ data: { pageId: identity.data.pageId, data: parsed.data.data as Prisma.InputJsonValue, ...toSeoData(parsed.data.seo) } });
...
async function saveDraft(pageId: string, data: StaticPageData, seo: ReturnType<typeof pageSeoSchema.parse>) {
  await prisma.$transaction(async (tx) => {
    const page = await tx.page.findUnique({ where: { id: pageId }, select: { draftVersionId: true } });
...
function revalidatePagePaths(key: PageKey) { revalidatePath("/dashboard/pages"); revalidatePath(`/dashboard/pages/${key}`); revalidatePath(pagePath(key)); revalidatePath("/sitemap.xml"); }
