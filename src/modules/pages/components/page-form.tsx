...
function HomeFields({ data, media, options }: { data: HomePageData | null; media: MediaPickerItem[]; options: PageFormProps["relationOptions"] }) {
  return <>
    <CmsFieldGroup title="الواجهة الرئيسية"><Text id="heroEyebrow" label="السطر التمهيدي" value={data?.hero.eyebrow} /><Text id="heroTitle" label="العنوان" value={data?.hero.title} /><Area id="heroDescription" label="الوصف" value={data?.hero.description} /><div className="grid gap-4 sm:grid-cols-2"><Text id="primaryCtaText" label="نص الإجراء الرئيسي" value={data?.hero.primaryCtaText} /><Text id="primaryCtaTarget" label="رابط الإجراء الرئيسي" value={data?.hero.primaryCtaTarget} ltr /></div><div className="grid gap-4 sm:grid-cols-2"><Text id="secondaryCtaText" label="نص الإجراء الثانوي" value={data?.hero.secondaryCtaText} /><Text id="secondaryCtaTarget" label="رابط الإجراء الثانوي" value={data?.hero.secondaryCtaTarget} ltr /></div><MediaPicker items={media} name="heroMediaId" defaultValue={data?.hero.mediaId} label="صورة الواجهة الرئيسية" /><Text id="heroImageAlt" label="النص البديل للصورة" value={data?.hero.imageAlt} /><div className="mt-2 grid gap-3 border-t border-border pt-5"><div><h3 className="text-base font-semibold">قيم الواجهة الرئيسية</h3><p className="mt-1 text-sm leading-[1.6] text-text-secondary">تظهر داخل الشريط السفلي للواجهة الرئيسية.</p></div><RepeatableItems name="trustItem" initialItems={data?.trustSection.items} addLabel="إضافة بند" maxItems={3} minimumRows={3} showIcon /></div></CmsFieldGroup>
