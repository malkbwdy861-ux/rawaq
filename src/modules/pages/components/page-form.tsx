...
function Area({ id, label, value, tall }: { id: string; label: string; value?: string | null; tall?: boolean }) { return <CmsFieldShell id={id} label={label}><textarea className={`${cmsInputClassName} ${tall ? "min-h-48" : "min-h-24"}`} id={id} name={id} defaultValue={value ?? ""} /></CmsFieldShell>; }
