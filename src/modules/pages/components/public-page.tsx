import { ArrowLeft, Check, ImageIcon, MapPin, MessageCircle, PencilRuler, Phone, Settings2, ShieldCheck, Sun, UsersRound } from "lucide-react";
...
import { FeaturedProjects, type FeaturedProjectItem } from "./featured-projects";
...
type Version = { title?: string | null; name?: string | null; slug?: string | null; shortDescription?: string | null; excerpt?: string | null; question?: string | null; answer?: string | null; city?: string | null; district?: string | null; category?: { id: string; name: string; slug: string; iconKey: string; isActive: boolean } | null; coverMedia?: Media | null; heroMedia?: Media | null };
type Entity = { id: string; draftVersion?: Version | null; publishedVersion?: Version | null };
export type ResolvedPageData = { heroMedia: Media | null; valuePropositionMedia: Media | null; sectionMedia: Record<string, Media>; services: Entity[]; solutions: Entity[]; materials: Entity[]; projects: Entity[]; articles: Entity[]; faqs: Entity[]; settings: { primaryPhone: string; secondaryPhone: string | null; whatsappNumber: string; email: string | null; address: string | null; businessHours: string | null; defaultSeoTitle: string | null; defaultSeoDescription: string | null; defaultOpenGraphImage?: { url: string } | null; defaultWhatsappText: string | null; defaultCtaText: string | null } | null };
...
export function PublicPage({ pageKey, data, resolved, preview = false }: { pageKey: "HOME" | "ABOUT" | "CONTACT" | "PRICES"; data: StaticPageData; resolved: ResolvedPageData; preview?: boolean }) {
  return <main className="min-h-screen bg-[oklch(97.5%_0.009_100)]">
    ...
    {pageKey !== "HOME" ? <StaticPublicPage data={data as AboutPageData | ContactPageData | PricesPageData} pageKey={pageKey} preview={preview} resolved={resolved} /> : null}
  </main>;
}
...
function HomeContent({ data, resolved, preview }: { data: HomePageData; resolved: ResolvedPageData; preview: boolean }) {
  ...
  const projectItems = projects.flatMap((item): FeaturedProjectItem[] => {
    const version = entityVersion(item, preview);
    if (!version?.title || !version.shortDescription || !version.coverMedia) return [];
    const location = [version.city, version.district].filter(Boolean).join(" · ");
    return [{
      id: item.id,
      title: version.title,
      shortDescription: version.shortDescription,
      href: !preview && version.slug ? cmsContentPath("/projects", version.slug) : null,
      image: { url: version.coverMedia.url, altText: version.coverMedia.altText || version.title },
      ...(location ? { location } : {}),
      ...(version.category?.isActive ? { category: { name: version.category.name, iconKey: version.category.iconKey } } : {}),
    }];
  });
  const faqs = faqItems(data.faqSection.selectedFaqIds, resolved.faqs, preview).slice(0, 10);
  const valuePropositionMedia = resolved.valuePropositionMedia ?? solutionItems.find((item) => item.image)?.image ?? null;
  const projectsDescription = legacyProjectPlaceholder(data.featuredProjects.description) ? "نعرض مجموعة مختارة من المشاريع التي تعكس جودة التنفيذ واهتمامنا بالتفاصيل في مختلف أنحاء جدة." : data.featuredProjects.description;
  return <>
    ...
    {data.featuredProjects.enabled ? <FeaturedProjects eyebrow={data.featuredProjects.eyebrow} title={data.featuredProjects.title || "مشاريع مختارة"} description={projectsDescription} ctaLabel={data.featuredProjects.ctaLabel} ctaHref={data.featuredProjects.ctaHref} items={projectItems} /> : null}
    {data.howWeWork.enabled ? <HowWeWorkSection data={data.howWeWork} media={resolved.sectionMedia} /> : null}
    {data.trustSection.enabled ? <TrustProofSection data={data.trustSection} media={resolved.sectionMedia} /> : null}
    {data.faqSection.enabled && faqs.length ? <HomeFaqSection data={data.faqSection} items={faqs} /> : null}
    {!preview && data.faqSection.enabled && faqs.length ? <FaqStructuredData items={faqs} /> : null}
    {data.finalCta.enabled ? <HomeFinalCta data={data.finalCta} media={resolved.sectionMedia} settings={resolved.settings} /> : null}
  </>;
}

const proofIcons = { location: MapPin, shield: ShieldCheck, team: UsersRound, settings: Settings2, climate: Sun, design: PencilRuler };

function HowWeWorkSection({ data, media }: { data: HomePageData["howWeWork"]; media: Record<string, Media> }) {
  const steps = data.steps.filter((step) => step.enabled && (step.title || step.description || step.mediaId)).sort((a, b) => a.order - b.order).slice(0, 4);
  if (!steps.length && !data.title) return null;
  return <section aria-labelledby="process-title" className="bg-background px-4 py-16 md:px-8 md:py-20 lg:py-24">
    <div className="mx-auto max-w-7xl">
      <header className="grid gap-5 md:grid-cols-12 md:items-end">
        <div className="md:col-span-7">
          ...
          {data.title ? <h2 className="mt-4 max-w-[720px] text-[clamp(2rem,3.4vw,3rem)] font-bold leading-[1.28] text-pretty" id="process-title">{data.title}</h2> : null}
        </div>
        {data.description ? <p className="max-w-[54ch] text-[1.0625rem] leading-[1.85] text-text-secondary md:col-span-5">{data.description}</p> : null}
      </header>
      {steps.length ? <ol className="mt-10 grid gap-y-10 sm:grid-cols-2 sm:gap-x-0 md:mt-12 lg:grid-cols-4">
        {steps.map((step, index) => {
          const image = step.mediaId ? media[step.mediaId] : null;
          return <li className={`relative min-w-0 sm:px-5 sm:first:pe-0 lg:px-6 lg:first:pe-0 ${index > 0 ? "sm:border-s sm:border-border" : ""} ${index === 2 ? "sm:border-s-0 lg:border-s" : ""}`} key={step.id}>
            {image ? <div className="relative aspect-[4/3] overflow-hidden rounded-[2px]"><Image alt={step.imageAlt || image.altText || step.title || ""} className="object-cover" fill sizes="(min-width:1024px) 25vw, (min-width:640px) 50vw, 100vw" src={image.url} /></div> : null}
            <div className={`${image ? "relative -mt-5" : "border-t border-border pt-4"}`}>
              <span className={`${image ? "grid size-12 place-items-center rounded-[9px] bg-primary text-primary-foreground" : "text-clay-strong"} inline-grid tabular-nums text-sm font-semibold`} dir="ltr">{String(index + 1).padStart(2, "0")}</span>
              {step.title ? <h3 className="mt-5 text-xl font-semibold leading-[1.5]">{step.title}</h3> : null}
              {step.description ? <p className="mt-2 max-w-[34ch] text-[0.9375rem] leading-[1.8] text-text-secondary">{step.description}</p> : null}
            </div>
          </li>;
        })}
      </ol> : null}
...
function TrustProofSection({ data, media }: { data: HomePageData["trustSection"]; media: Record<string, Media> }) {
  const metrics = data.metrics.filter((item) => item.enabled && (item.title || item.value || item.description)).sort((a, b) => a.order - b.order).slice(0, 4);
  const strip = data.stripItems.filter((item) => item.enabled && item.title).sort((a, b) => a.order - b.order).slice(0, 3);
  const featuredImage = data.featuredProof.mediaId ? media[data.featuredProof.mediaId] : null;
  if (!data.title && !data.featuredProof.title && !metrics.length) return null;
  return <section aria-labelledby="trust-proof-title" className="bg-muted px-4 py-16 md:px-8 md:py-20 lg:py-24">
    <div className="mx-auto max-w-7xl">
      <header className="max-w-3xl">
        {data.eyebrow ? <p className="flex items-center gap-3 text-sm font-semibold text-clay-strong before:h-px before:w-9 before:bg-clay">{data.eyebrow}</p> : null}
        <h2 className="mt-4 text-[clamp(2rem,3.4vw,3rem)] font-bold leading-[1.28] text-pretty" id="trust-proof-title">{data.title || "ثقة تُبنى على تفاصيل واضحة"}</h2>
      ...
      <div className="mt-10 grid gap-4 lg:grid-cols-12 lg:gap-5">
        {(data.featuredProof.title || data.featuredProof.description || featuredImage) ? <article className="relative isolate flex min-h-[360px] overflow-hidden rounded-[12px] bg-primary-active p-6 text-primary-foreground sm:min-h-[420px] sm:p-8 lg:col-span-7 lg:p-10">
          {featuredImage ? <Image alt={featuredImage.altText || data.featuredProof.title || ""} className="-z-20 object-cover" fill sizes="(min-width:1024px) 58vw, 100vw" src={featuredImage.url} /> : <div aria-hidden="true" className="absolute inset-0 -z-20 bg-[linear-gradient(135deg,var(--primary-active),var(--primary))] [background-image:linear-gradient(135deg,var(--primary-active),var(--primary)),linear-gradient(90deg,color-mix(in_oklch,var(--primary-soft)_13%,transparent)_1px,transparent_1px)] [background-size:auto,64px_100%]" />}
          <div aria-hidden="true" className="absolute inset-0 -z-10 bg-[linear-gradient(to_top,color-mix(in_oklch,var(--primary-active)_94%,transparent),color-mix(in_oklch,var(--primary-active)_26%,transparent))]" />
          <div className="mt-auto max-w-[560px]"><ShieldCheck className="size-8 text-clay" strokeWidth={1.5} />{data.featuredProof.title ? <h3 className="mt-5 text-[clamp(1.75rem,3vw,2.5rem)] font-bold leading-[1.35]">{data.featuredProof.title}</h3> : null}{data.featuredProof.description ? <p className="mt-3 max-w-[50ch] leading-[1.8] text-primary-soft">{data.featuredProof.description}</p> : null}</div>
        </article> : null}
        {metrics.length ? <div className="grid gap-4 sm:grid-cols-2 lg:col-span-5">
          {metrics.map((item) => { const Icon = proofIcons[item.icon ?? "shield"]; return <article className="flex min-h-[190px] flex-col border-t border-border-strong bg-background p-5 sm:min-h-[200px]" key={item.id}><Icon aria-hidden="true" className="size-5 text-clay-strong" strokeWidth={1.6} />{item.value ? <p className="mt-auto text-3xl font-bold tabular-nums text-primary" dir="auto">{item.value}</p> : null}<h3 className={`${item.value ? "mt-2" : "mt-auto"} text-lg font-semibold leading-[1.5]`}>{item.title}</h3>{item.description ? <p className="mt-2 text-sm leading-[1.75] text-text-secondary">{item.description}</p> : null}</article>; })}
        </div> : null}
      </div>
      {strip.length ? <ul className="mt-7 grid border-y border-border sm:grid-cols-3">{strip.map((item, index) => { const Icon = proofIcons[item.icon ?? "shield"]; return <li className={`flex min-h-16 items-center gap-3 py-4 sm:px-5 sm:first:pe-0 ${index > 0 ? "border-t border-border sm:border-s sm:border-t-0" : ""}`} key={item.id}><Icon aria-hidden="true" className="size-[18px] shrink-0 text-primary" strokeWidth={1.7} /><span className="text-sm font-semibold">{item.title}</span></li>; })}</ul> : null}
    </div>
  </section>;
}

function HomeFaqSection({ data, items }: { data: HomePageData["faqSection"]; items: { id: string; question: string | null; answer: string | null }[] }) {
  return <section aria-labelledby="home-faq-title" className="bg-background px-4 py-16 md:px-8 md:py-20 lg:py-24" id="faq">
    <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-12 lg:gap-16">
      <header className="lg:col-span-5">
        {data.eyebrow ? <p className="flex items-center gap-3 text-sm font-semibold text-clay-strong before:h-px before:w-9 before:bg-clay">{data.eyebrow}</p> : null}
        <h2 className="mt-4 max-w-[560px] text-[clamp(2.25rem,4.3vw,3.75rem)] font-bold leading-[1.2] text-pretty" id="home-faq-title">{data.title || "إجابات واضحة قبل بدء مشروعك"}</h2>
        {data.description ? <p className="mt-5 max-w-[46ch] text-[1.0625rem] leading-[1.85] text-text-secondary">{data.description}</p> : null}
        <div aria-hidden="true" className="mt-10 hidden h-32 w-3/4 border-y border-border bg-[repeating-linear-gradient(90deg,transparent_0,transparent_31px,var(--border)_32px)] lg:block" />
      </header>
      <div className="lg:col-span-7"><HomeFaqAccordion items={items} />{data.allFaqsLabel && data.allFaqsHref ? <Link className="group mt-7 inline-flex min-h-12 items-center gap-3 text-sm font-semibold text-primary underline decoration-border-strong underline-offset-8 hover:decoration-primary focus-visible:outline-ring" href={data.allFaqsHref}>{data.allFaqsLabel}<ArrowLeft aria-hidden="true" className="size-[18px] transition-transform duration-300 group-hover:-translate-x-1" /></Link> : null}</div>
    </div>
  </section>;
}

function HomeFinalCta({ data, media, settings }: { data: HomePageData["finalCta"]; media: Record<string, Media>; settings: ResolvedPageData["settings"] }) {
  if (!data.title) return null;
  const background = data.backgroundMediaId ? media[data.backgroundMediaId] : null;
  const primaryHref = settings?.whatsappNumber ? buildWhatsAppUrl(settings.whatsappNumber, settings.defaultWhatsappText) : data.target;
  const secondaryHref = settings?.primaryPhone ? phoneHref(settings.primaryPhone) : null;
  const primaryLabel = data.primaryCtaLabel || data.buttonText;
  const trustItems = data.trustItems.filter((item) => item.enabled && item.title).sort((a, b) => a.order - b.order).slice(0, 3);
  return <section className="bg-background px-4 pb-16 pt-4 md:px-8 md:pb-20 lg:pb-24" aria-labelledby="final-cta-title">
    <div className="relative isolate mx-auto max-w-7xl overflow-hidden rounded-[16px] bg-primary-active px-5 py-12 text-primary-foreground sm:px-8 md:py-16 lg:px-16 lg:py-20">
      {background ? <Image alt="" className="-z-20 object-cover object-center" fill sizes="(min-width:1280px) 1280px, 100vw" src={background.url} /> : <div aria-hidden="true" className="absolute inset-0 -z-20 bg-[radial-gradient(circle_at_12%_25%,color-mix(in_oklch,var(--primary)_70%,transparent),transparent_38%),linear-gradient(125deg,var(--primary-active),var(--primary-hover))]" />}
      <div aria-hidden="true" className="absolute inset-0 -z-10 bg-[linear-gradient(90deg,color-mix(in_oklch,var(--primary-active)_25%,transparent),color-mix(in_oklch,var(--primary-active)_90%,transparent))]" />
      <div className="mx-auto max-w-3xl text-center">
        {data.eyebrow ? <p className="text-sm font-semibold text-clay">{data.eyebrow}</p> : null}
        <h2 className="mt-4 text-[clamp(2rem,4vw,3.25rem)] font-bold leading-[1.25] text-pretty" id="final-cta-title">{data.title}</h2>
        {data.description ? <p className="mx-auto mt-4 max-w-[52ch] text-[1.0625rem] leading-[1.85] text-primary-soft">{data.description}</p> : null}
        <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
          {primaryLabel && primaryHref ? <a className="inline-flex min-h-12 items-center justify-center gap-2 rounded-[9px] bg-primary-foreground px-5 text-sm font-semibold text-primary-active transition-colors hover:bg-primary-soft focus-visible:outline-ring" href={primaryHref}><MessageCircle aria-hidden="true" className="size-[18px]" />{primaryLabel}</a> : null}
          {data.secondaryCtaLabel && secondaryHref ? <a className="inline-flex min-h-12 items-center justify-center gap-2 rounded-[9px] border border-primary-soft/55 px-5 text-sm font-semibold text-primary-foreground transition-colors hover:border-primary-soft hover:bg-primary/35 focus-visible:outline-ring" href={secondaryHref}><Phone aria-hidden="true" className="size-[18px]" />{data.secondaryCtaLabel}</a> : null}
        </div>
      </div>
      {trustItems.length ? <ul className="mx-auto mt-9 flex max-w-2xl flex-wrap justify-center gap-x-7 gap-y-3 border-t border-primary-soft/20 pt-5">{trustItems.map((item) => <li className="flex items-center gap-2 text-xs font-semibold text-primary-soft" key={item.id}><Check aria-hidden="true" className="size-4 text-clay" />{item.title}</li>)}</ul> : null}
    </div>
  </section>;
}
...
export function AboutContent({ data, resolved }: { data: AboutPageData; resolved: ResolvedPageData }) {
...
export function ContactContent({ data, resolved }: { data: ContactPageData; resolved: ResolvedPageData }) {
...
export function PricesContent({ data, resolved, preview }: { data: PricesPageData; resolved: ResolvedPageData; preview: boolean }) {
...
function legacyProjectPlaceholder(description?: string) { return Boolean(description && /(عند إضافة مشاريع|ستظهر هنا)/.test(description)); }
