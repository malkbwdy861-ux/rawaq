...
export function SolutionCard({ item, index, total = 1, active = true, variant = "archive", panelRef }: { item: FeaturedSolutionItem; index: number; total?: number; active?: boolean; variant?: "mobile" | "desktop" | "archive"; panelRef?: (node: HTMLElement | null) => void }) {
  if (variant === "mobile") {
    return <article aria-hidden={!active} aria-label={`${index + 1} من ${total}: ${item.title}`} aria-roledescription="شريحة" className="relative isolate h-full w-full shrink-0 overflow-hidden" dir="rtl" inert={!active}>
      ...
      <div className="relative z-10 flex h-full flex-col justify-between px-5 pb-24 pt-7 sm:px-8 sm:pb-28"><p className="flex items-center gap-3 text-sm font-semibold text-primary-soft before:h-px before:w-8 before:bg-clay">الحلول المميزة</p><SolutionCardText item={item} index={index} total={total} variant="mobile" /></div>
    </article>;
  }
...
function SolutionCardText({ item, index, total, variant }: { item: FeaturedSolutionItem; index: number; total: number; variant: "mobile" | "desktop" | "archive" }) {
  ...
  return <div className={variant === "mobile" ? "max-w-[580px] text-right" : undefined}>
    <p className={`${variant === "archive" ? "mb-2 text-xs" : "mb-4 text-sm"} text-start font-semibold tabular-nums text-primary-soft`}><bdi dir="ltr">{number(index + 1)} / {number(total)}</bdi></p>
    ...
    <p className={`${variant === "desktop" ? "mt-5 max-w-[58ch] text-[1.0625rem] leading-[1.85] xl:text-lg" : variant === "mobile" ? "mt-4 line-clamp-3 max-w-[48ch] text-[1.0625rem] leading-[1.8]" : "mt-2 line-clamp-2 max-w-[48ch] text-sm leading-[1.75]"} text-primary-soft`}>{item.shortDescription}</p>
...
function SolutionLink({ href, compact = false }: { href: string | null; compact?: boolean }) {
  const className = `group ${compact ? "mt-4 min-h-11 px-4" : "mt-7 min-h-12 px-5"} inline-flex w-fit items-center gap-3 rounded-[9px] border border-primary-soft/55 text-sm font-semibold text-primary-foreground transition-colors duration-150 hover:border-primary-soft hover:bg-primary focus-visible:outline-primary-soft`;
