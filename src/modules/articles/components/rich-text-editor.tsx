...
const controlClass = "min-h-10 rounded-[4px] border border-control-border bg-card px-3 py-2 text-sm font-semibold transition-colors hover:bg-secondary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring disabled:text-text-disabled aria-pressed:border-brand-accent-strong aria-pressed:bg-brand-secondary-soft";
...
export function RichTextEditor({ defaultValue, error }: { defaultValue?: TipTapDocument | null; error?: string }) {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: "min-h-72 max-w-[72ch] px-4 py-4 text-base leading-[1.8] outline-none [&_blockquote]:border-y [&_blockquote]:border-border [&_blockquote]:py-3 [&_h2]:mt-6 [&_h2]:text-2xl [&_h2]:font-bold [&_h3]:mt-5 [&_h3]:text-xl [&_h3]:font-semibold [&_ol]:list-decimal [&_ol]:pr-6 [&_p]:my-3 [&_ul]:list-disc [&_ul]:pr-6",
  ...
  return (
    <div className="overflow-hidden rounded-[8px] border border-control-border bg-card focus-within:border-brand-accent-strong focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2">
      <input name="content" type="hidden" value={JSON.stringify(content)} />
      <div aria-label="تنسيق المحتوى" className="flex flex-wrap gap-2 border-b border-border bg-surface-raised p-3" role="toolbar">
        ...
        <div className="flex min-w-full gap-2 sm:min-w-0 sm:flex-1">
          ...
          <Input aria-describedby={linkError ? "article-link-error" : undefined} aria-invalid={Boolean(linkError)} className="min-h-10 min-w-0 flex-1 rounded-[4px] border-control-border bg-card text-sm aria-invalid:border-destructive" dir="ltr" id="article-link" onChange={(event) => setLinkHref(event.target.value)} placeholder="https:// أو /guides/..." type="text" value={linkHref} />
          <button className={controlClass} disabled={!editor} onClick={applyLink} type="button">تطبيق الرابط</button>
        </div>
        {linkError ? <p className="min-w-full text-sm font-medium text-destructive" id="article-link-error">{linkError}</p> : null}
      </div>
