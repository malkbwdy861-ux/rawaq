...
const articleTypes = [
  ["NONE", "بدون تصنيف"], ["GUIDE", "مقال إرشادي"], ["PRICING", "مقال أسعار"], ["COMPARISON", "مقارنة"], ["MAINTENANCE", "صيانة"], ["GENERAL", "مقال عام"],
] as const;
...
export function ArticleForm({ article, media, relationOptions }: { article?: { id: string; status: "DRAFT" | "PUBLISHED" | "ARCHIVED"; publishedVersionId: string | null; publishedAt: Date | null; updatedAt: Date; draftVersion: ArticleFormVersion | null }; media: MediaPickerItem[]; relationOptions: RelationOptions }) {
  ...
  const title = article ? values.title || draft?.title || "مقال بدون عنوان" : "مقال جديد";
      ...
      <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,820px)_300px]" key={state.revision}>
        <div className="rounded-xl border border-border bg-card px-5 sm:px-7">
          <EditorSection title="المعلومات الأساسية">
            <CmsFieldShell error={fieldError(state, "title")} id="title" label="عنوان المقال"><CmsInput aria-describedby={fieldError(state, "title") ? "title-error" : undefined} aria-invalid={Boolean(fieldError(state, "title"))} autoFocus className="min-h-12 text-lg font-semibold" id="title" name="title" defaultValue={values.title} placeholder="مثال: كيف تختار مظلة السيارة المناسبة" /></CmsFieldShell>
            ...
            <CmsFieldShell error={fieldError(state, "articleType")} id="articleType" label="نوع المقال"><Select defaultValue={values.articleType || "NONE"} name="articleType"><SelectTrigger aria-invalid={Boolean(fieldError(state, "articleType"))} className="min-h-11 max-w-[760px] border-border-strong bg-card text-base focus-visible:border-primary focus-visible:ring-ring focus-visible:ring-offset-2" id="articleType"><SelectValue /></SelectTrigger><SelectContent align="end">{articleTypes.map(([value, label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select></CmsFieldShell>
          </EditorSection>

          <EditorSection title="المحتوى">
            <CmsFieldShell error={fieldError(state, "content")} hint="التنسيقات المتاحة محصورة في العناوين والقوائم والاقتباس والروابط الآمنة." id="article-content" label="محتوى المقال"><RichTextEditor defaultValue={editorDocument} error={fieldError(state, "content")} /></CmsFieldShell>
          </EditorSection>
        ...
        <aside className="grid min-w-0 gap-3 [&>details]:order-2 xl:sticky xl:top-20 xl:[&>details]:order-3">
          <section className="order-3 min-w-0 rounded-xl border border-border bg-card p-4 xl:order-1">
            ...
            {article ? <dl className="mt-4 divide-y divide-border text-xs"><MetaRow label="آخر حفظ" value={(draft?.updatedAt ?? article.updatedAt).toLocaleString("ar-SA", { dateStyle: "medium", timeStyle: "short" })} /><MetaRow label="الظهور" value={article.publishedVersionId && article.status === "PUBLISHED" ? "منشور للعامة" : "غير منشور للعامة"} />{article.publishedAt ? <MetaRow label="آخر نشر" value={article.publishedAt.toLocaleDateString("ar-SA", { dateStyle: "medium" })} /> : null}</dl> : <p className="mt-3 text-xs leading-5 text-text-secondary">سيُنشأ السجل فقط عند الحفظ أو النشر.</p>}
          </section>
