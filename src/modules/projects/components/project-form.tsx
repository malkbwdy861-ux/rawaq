  ...
  return (
    <form action={formAction} noValidate ref={formRef}>
      ...
      <header className="mb-6 border-b border-border pb-5">
        <Link className="inline-flex min-h-8 items-center text-xs font-medium text-muted-foreground outline-none transition-colors hover:text-brand-accent-strong focus-visible:ring-2 focus-visible:ring-ring" href="/dashboard/projects">المشاريع <span aria-hidden="true" className="mx-1.5">/</span> <span className="text-foreground">{title}</span></Link>
      ...
      <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,820px)_300px]" key={state.revision}>
        ...
        <aside className="grid min-w-0 gap-3 [&>details]:order-2 xl:sticky xl:top-20 xl:[&>details]:order-3">
          <section className="order-3 min-w-0 rounded-xl border border-border bg-card p-4 xl:order-1">
            ...
            {project ? <dl className="mt-4 divide-y divide-border text-xs"><MetaRow label="آخر حفظ" value={(draft?.updatedAt ?? project.updatedAt).toLocaleString("ar-SA", { dateStyle: "medium", timeStyle: "short" })} /><MetaRow label="الظهور" value={project.publishedVersionId ? "منشور للعامة" : "غير منشور للعامة"} />{project.publishedAt ? <MetaRow label="آخر نشر" value={project.publishedAt.toLocaleDateString("ar-SA", { dateStyle: "medium" })} /> : null}</dl> : <p className="mt-3 text-xs leading-5 text-text-secondary">سيُنشأ السجل عند الحفظ أو النشر.</p>}
          </section>
      ...
      <div className="sticky bottom-4 z-10 mt-5 flex flex-col gap-3 rounded-xl border border-border bg-card p-3 shadow-[var(--shadow-float)] sm:flex-row sm:items-center sm:justify-between">
        <p className="hidden text-sm leading-6 text-text-secondary sm:block">احفظ المشروع كمسودة للمراجعة، أو انشره عند اكتمال المحتوى والصور.</p>
        <div className="flex flex-col-reverse gap-2 sm:flex-row sm:items-center">
          {project ? <Link className={cn(buttonVariants({ variant: "outline" }), "min-h-11 w-full px-5 sm:w-auto")} href={`/preview/projects/${project.id}`} target="_blank">معاينة<ExternalLink /></Link> : null}
