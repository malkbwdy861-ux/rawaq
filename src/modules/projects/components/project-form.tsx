  ...
  return (
    <form className="space-y-8" action={saveProjectDraftAction}>
      ...
      <CmsFieldGroup title="محتوى المشروع" description="العنوان والوصف المختصر مطلوبان عند النشر. يتم إنشاء الرابط المختصر تلقائيًا من العنوان ويمكن حفظ المسودة قبل اكتمالها.">
        <CmsFieldShell id="title" label="عنوان المشروع"><CmsInput id="title" name="title" defaultValue={draft?.title ?? ""} /></CmsFieldShell>
      ...
      <CmsFieldGroup title="وقائع المشروع" description="اترك أي معلومة غير موثقة فارغة، ولا تستنتج الموقع أو النطاق أو التفاصيل الفنية.">
        <div className="grid gap-4 sm:grid-cols-2">
          <CmsFieldShell id="city" label="المدينة"><CmsInput id="city" name="city" defaultValue={draft?.city ?? ""} /></CmsFieldShell>
          <CmsFieldShell id="district" label="الحي"><CmsInput id="district" name="district" defaultValue={draft?.district ?? ""} /></CmsFieldShell>
        </div>
        <CmsFieldShell id="completedAt" label="تاريخ الإنجاز"><CmsInput dir="ltr" id="completedAt" name="completedAt" type="date" defaultValue={draft?.completedAt ? draft.completedAt.toISOString().slice(0, 10) : ""} /></CmsFieldShell>
