...
export default async function ServicesIndexPage({ searchParams }: { searchParams: Promise<{ page?: string | string[] }> }) {
  const page = parsePublicPage((await searchParams).page);
  const [pageContent, result, settings] = await Promise.all([getPublishedPage("SERVICES"), getPublishedServices(page), getSiteSettings()]);
  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      <div className="public-container pb-16 md:pb-20 lg:pb-24">
        {items.length ? <><section aria-label="قائمة الخدمات" className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 lg:gap-5">{items.map((item, index) => <ServiceCard index={index} item={item} key={item.id} />)}</section><div className="mt-10"><CmsPaginationControls basePath="/services" pagination={result.pagination} /></div></> : <section className="border-y border-border py-12"><h2 className="text-2xl font-semibold">نعمل على تحديث تفاصيل الخدمات</h2><p className="mt-3 text-text-secondary">يمكنك التواصل معنا مباشرة لمناقشة احتياج موقعك.</p></section>}
      </div>
