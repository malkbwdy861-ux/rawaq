...
export default async function SolutionDetailPage({ params }: SolutionPageProps) {
  ...
  return <main className="min-h-screen bg-[oklch(97.5%_0.009_100)] pb-14 md:pb-20">
    <article>
      <section className="relative isolate overflow-hidden bg-primary-active px-4 pb-10 pt-8 text-primary-foreground md:px-8 md:pb-16 md:pt-12 lg:pb-20">
        <div aria-hidden="true" className="absolute inset-0 bg-[radial-gradient(ellipse_80%_70%_at_4%_8%,color-mix(in_oklch,var(--primary)_58%,transparent),transparent_65%),linear-gradient(135deg,color-mix(in_oklch,var(--primary)_24%,var(--primary-active)),var(--primary-active))]" />
        <DetailContainer className="relative z-10 space-y-8">
          ...
          <header className="grid gap-8 lg:grid-cols-12 lg:items-center">
            <div className="max-w-4xl space-y-5 lg:col-span-7">
              <p className="text-sm font-semibold text-primary-soft">حل عملي للمساحة</p>
              <h1 className="max-w-[14ch] text-[clamp(2.25rem,5vw,4.45rem)] font-bold leading-[1.22] text-pretty">{version.title}</h1>
              <p className="max-w-[58ch] text-lg leading-[1.9] text-primary-soft/88 md:text-xl">{version.shortDescription}</p>
              <div className="flex flex-col gap-3 pt-2 sm:flex-row">
                <a className="inline-flex min-h-12 items-center justify-center gap-2 rounded-[9px] bg-primary-foreground px-5 text-sm font-semibold text-primary-active transition-colors hover:bg-primary-soft" href={whatsappHref}><MessageCircle aria-hidden="true" className="size-[18px]" />اسأل عن هذا الحل</a>
                <Link className="group inline-flex min-h-12 items-center justify-center gap-2 rounded-[9px] border border-primary-soft/45 px-5 text-sm font-semibold text-primary-foreground transition-colors hover:border-primary-soft hover:bg-primary/35" href="/projects">شاهد تطبيقات مشابهة<ArrowLeft aria-hidden="true" className="size-4 transition-transform group-hover:-translate-x-1" /></Link>
              </div>
            </div>
            <figure className="lg:col-span-5">
              ...
              {version.heroMedia?.caption ? <figcaption className="mt-3 text-sm leading-[1.7] text-primary-soft/75">{version.heroMedia.caption}</figcaption> : null}
            </figure>
          </header>
        </DetailContainer>
      </section>
