...
export default async function ArticlePreviewPage({ params }: ArticlePreviewPageProps) {
  ...
  return <main className="min-h-screen bg-[oklch(97.5%_0.009_100)] px-4 py-10 md:px-8"><article className="mx-auto max-w-6xl space-y-12">
    ...
    <header className="max-w-4xl space-y-5"><p className="text-sm font-semibold text-[oklch(58%_0.11_45)]">{draft.articleType ? typeLabels[draft.articleType] : "دليل"}</p><h1 className="text-4xl font-bold leading-[1.24] md:text-6xl">{draft.title ?? "مقال بدون عنوان"}</h1>{draft.excerpt ? <p className="max-w-[62ch] text-xl leading-[1.7] text-[oklch(42%_0.018_150)]">{draft.excerpt}</p> : null}</header>
