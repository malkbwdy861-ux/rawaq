    ...
    <div className="mx-auto w-full max-w-[1600px] space-y-5">
      ...
      <details className="group scroll-mt-20 rounded-xl border border-border bg-card shadow-[var(--shadow-rest)]" id="media-upload" open={mediaItems.length === 0}>
        ...
        <form action={uploadMediaAction} className="grid gap-3 border-t border-border p-5 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
          <label className="grid gap-2"><span className="text-xs font-semibold">اختر الصورة</span><input accept={mediaConfig.allowedMimeTypes.join(",")} className="min-h-11 rounded-md border border-border-strong bg-card px-3 py-2 text-sm file:me-3 file:min-h-8 file:rounded-md file:border-0 file:bg-secondary file:px-3 file:text-xs file:font-semibold focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring" name="file" required type="file" /></label>
      ...
      <section aria-label="أدوات مكتبة الوسائط" className="rounded-xl border border-border bg-card p-3 shadow-[var(--shadow-rest)]">
        <form action="" className="grid gap-2 lg:grid-cols-[minmax(260px,1fr)_180px_180px_auto]">
          <label className="relative"><span className="sr-only">البحث في الوسائط</span><Search className="pointer-events-none absolute start-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><input className="h-10 w-full rounded-md border border-border-strong bg-card ps-9 pe-3 text-sm outline-none placeholder:text-muted-foreground focus:border-primary focus:ring-2 focus:ring-ring/25" defaultValue={query} name="q" placeholder="ابحث باسم الملف أو الوصف..." type="search" /></label>
          <label className="relative"><span className="sr-only">نوع الوسائط</span><select className="h-10 w-full appearance-none rounded-md border border-border-strong bg-card px-3 pe-9 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-ring/25" defaultValue={type ?? "ALL"} name="type"><option value="ALL">كل الأنواع</option><option value={MediaType.IMAGE}>صور</option><option value={MediaType.FILE}>ملفات</option></select><ChevronDown className="pointer-events-none absolute end-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /></label>
          <label className="relative"><span className="sr-only">ترتيب الوسائط</span><select className="h-10 w-full appearance-none rounded-md border border-border-strong bg-card px-3 pe-9 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-ring/25" defaultValue={sort} name="sort"><option value="latest">الأحدث</option><option value="oldest">الأقدم</option><option value="name">اسم الملف</option></select><ChevronDown className="pointer-events-none absolute end-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /></label>
