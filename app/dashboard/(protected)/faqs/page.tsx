...
import { CreateFaqDialog } from "@/modules/faqs/components/create-faq-dialog";
  ...
  return <div className="mx-auto w-full max-w-[1360px] space-y-5">
    ...
    <section aria-label="مجموعة الأسئلة الشائعة" className="overflow-hidden rounded-xl border border-border bg-card shadow-[var(--shadow-rest)]">
      ...
      {faqs.length === 0 ? <div className="grid min-h-72 place-items-center px-6 py-12 text-center"><div><span className="mx-auto grid size-10 place-items-center rounded-md bg-secondary text-muted-foreground"><FileQuestion className="size-5" /></span><h2 className="mt-4 text-lg font-semibold">{filtered ? "لا توجد نتائج مطابقة" : "لا توجد أسئلة بعد"}</h2><p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-text-secondary">{filtered ? "غيّر عبارة البحث أو الحالة لعرض أسئلة أخرى." : "أنشئ أول سؤال مع إجابته، ثم انشره عندما يصبح جاهزاً للظهور."}</p>{filtered ? <Link className="mt-4 inline-flex min-h-10 items-center text-sm font-semibold text-primary underline underline-offset-4" href="/dashboard/faqs">عرض كل الأسئلة</Link> : <div className="mt-5"><CreateFaqDialog compact /></div>}</div></div> : <>
        <div className="hidden md:block"><table className="w-full table-fixed text-start text-sm"><thead className="bg-dashboard-canvas/65 text-xs font-medium text-muted-foreground"><tr className="h-11"><th className="w-auto px-4 text-start font-medium">السؤال</th><th className="w-24 px-4 text-start font-medium">الترتيب</th><th className="w-52 px-4 text-start font-medium">الحالة</th><th className="w-40 px-4 text-start font-medium">آخر تحديث</th><th className="w-16 px-3"><span className="sr-only">الإجراءات</span></th></tr></thead><tbody>{faqs.map((faq) => { const version = faq.draftVersion ?? faq.publishedVersion; return <tr className="h-16 border-t border-border/80 transition-colors hover:bg-dashboard-hover/55" key={faq.id}><td className="px-4"><Link className="block min-w-0 py-2 outline-none focus-visible:ring-2 focus-visible:ring-ring" href={`/dashboard/faqs/${faq.id}`}><span className="block truncate font-semibold text-foreground">{version?.question ?? "سؤال بدون عنوان"}</span><span className="mt-0.5 block truncate text-xs font-normal text-muted-foreground">{version?.answer ?? "لا توجد إجابة في هذه النسخة"}</span></Link></td><td className="px-4 tabular-nums text-text-secondary">{version?.sortOrder?.toLocaleString("ar-SA") ?? "غير محدد"}</td><td className="px-4"><FaqStatus draft={Boolean(faq.draftVersionId)} published={Boolean(faq.publishedVersionId)} status={faq.status} /></td><td className="px-4 text-xs text-text-secondary"><time dateTime={faq.updatedAt.toISOString()}>{faq.updatedAt.toLocaleDateString("ar-SA", { day: "numeric", month: "short", year: faq.updatedAt.getFullYear() === new Date().getFullYear() ? undefined : "numeric" })}</time></td><td className="px-3"><Link aria-label={`تحرير ${version?.question ?? "السؤال"}`} className={cn(buttonVariants({ variant: "ghost", size: "icon" }), "size-9 min-h-9 text-muted-foreground")} href={`/dashboard/faqs/${faq.id}`} title="تحرير السؤال"><Pencil /></Link></td></tr>; })}</tbody></table></div>
        <div className="divide-y divide-border md:hidden">{faqs.map((faq) => { const version = faq.draftVersion ?? faq.publishedVersion; return <article className="grid grid-cols-[minmax(0,1fr)_auto] gap-3 px-4 py-4" key={faq.id}><div className="min-w-0 text-start"><Link className="block truncate font-semibold" href={`/dashboard/faqs/${faq.id}`}>{version?.question ?? "سؤال بدون عنوان"}</Link>{version?.answer ? <p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{version.answer}</p> : null}<div className="mt-3 flex flex-wrap items-center gap-3"><FaqStatus draft={Boolean(faq.draftVersionId)} published={Boolean(faq.publishedVersionId)} status={faq.status} /><time className="text-xs text-muted-foreground" dateTime={faq.updatedAt.toISOString()}>{faq.updatedAt.toLocaleDateString("ar-SA")}</time>{version?.sortOrder !== null && version?.sortOrder !== undefined ? <span className="text-xs tabular-nums text-muted-foreground">الترتيب {version.sortOrder.toLocaleString("ar-SA")}</span> : null}</div></div><Link aria-label={`تحرير ${version?.question ?? "السؤال"}`} className={cn(buttonVariants({ variant: "ghost", size: "icon" }), "size-9 min-h-9 text-muted-foreground")} href={`/dashboard/faqs/${faq.id}`}><Pencil /></Link></article>; })}</div>
      </>}
...
function FaqStatus({ status, draft, published }: { status: "DRAFT" | "PUBLISHED" | "ARCHIVED"; draft: boolean; published: boolean }) {
  const changed = status === "PUBLISHED" && draft && published;
  const label = changed ? "منشور مع تعديلات" : status === "PUBLISHED" ? "منشور" : status === "ARCHIVED" ? "مؤرشف" : "مسودة";
  const tone = status === "PUBLISHED" ? "bg-success-soft text-success" : status === "ARCHIVED" ? "bg-danger-soft text-destructive" : "bg-secondary text-text-secondary";
